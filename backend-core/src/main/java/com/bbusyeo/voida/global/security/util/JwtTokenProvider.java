...
import com.bbusyeo.voida.api.auth.domain.enums.JwtToken;
...
public class JwtTokenProvider {
    ...
    public JwtTokenProvider(@Value("${jwt.secret}") String secretKey, // application.yaml 에서 secret 값 추출
                            RedisDao redisDao) {
    ...
    @Value("${jwt.refresh-token.refresh-token}")
    ...
    private String generateAccessToken(String username, String authorities, Date expireDate) {
        return Jwts.builder()
                ...
                .signWith(secretKey, SignatureAlgorithm.HS256) // 지정된 키와 알고리즘으로 서명
    ...
    public String getUserNameFromToken(String token) {
        try {
            return Jwts.parserBuilder()
                    ...
                    .build()
                    .parseClaimsJws(token)
                    .getBody().getSubject(); // 사용자 이름(subject) 반환
        } catch (ExpiredJwtException e){
