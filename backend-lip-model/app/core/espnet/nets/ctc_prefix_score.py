...
@numba.jit(nopython=True, cache=True, nogil=False)
def _forward_ctc(r, log_phi, xs, x_blank, input_length, start):
    """Numba-jitted forward pass for CTC prefix score calculation."""
    log_psi = r[start - 1, 0].copy()  # Ensure we don't modify the original array view
    for t in range(start, input_length):
        r_t_n = numpy.logaddexp(r[t - 1, 0], log_phi[t - 1]) + xs[t]
        r_t_b = numpy.logaddexp(r[t - 1, 0], r[t - 1, 1]) + x_blank[t]
        r[t, 0] = r_t_n
        r[t, 1] = r_t_b
        log_psi = numpy.logaddexp(log_psi, log_phi[t - 1] + xs[t])
    return log_psi, r
...
class CTCPrefixScore(object):
    ...
    def initial_state(self):
        ...
        r = self.xp.full((self.input_length, 2), self.logzero, dtype=numpy.float32)
    ...
    def __call__(self, y, cs, r_prev):
        ...
        r = self.xp.full(
            (self.input_length, 2, len(cs)), self.logzero, dtype=numpy.float32
        )
        xs = self.x[:, cs]
        if output_length == 0:
            r[0, 0] = xs[0]
        ...
        if output_length > 0 and last in cs:
            log_phi = self.xp.full(
                (self.input_length, len(cs)), self.logzero, dtype=numpy.float32
            )
        ...
        # Call the JIT-compiled function for the computationally intensive part
        log_psi, r = _forward_ctc(
            r, log_phi, xs, self.x[:, self.blank], self.input_length, start
        )
