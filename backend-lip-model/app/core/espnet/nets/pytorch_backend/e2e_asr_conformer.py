...
class E2E(torch.nn.Module):
    ...
    def decode(self, hs_pad: torch.Tensor) -> str:
        """
        Performs beam search decoding on the given encoded features.
        Used for inference.

        Args:
            hs_pad (torch.Tensor): The encoded features from the encoder.

        Returns:
            str: The decoded transcript.
        """
        from .nets_utils import get_beam_search_decoder
        from ....datamodule.transforms import TextTransform

        text_transform = TextTransform()
        beam_search = get_beam_search_decoder(
            self,
            text_transform.token_list,
            ctc_weight=0.1,  # Revert to the model's default weight
        )

        # hs_pad comes from OpenVINO, ensure it's on the right device
        hs_pad = hs_pad.to(next(self.decoder.parameters()).device)

        enc_feat = hs_pad.squeeze(0)
