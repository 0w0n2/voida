...
def cut_patch(img, landmarks, height, width, threshold=5):
    # Numba-friendly equivalent of np.mean(landmarks, axis=0)
    center_x = landmarks[:, 0].sum() / landmarks.shape[0]
    center_y = landmarks[:, 1].sum() / landmarks.shape[0]
    ...
    y_min = int(round(max(0, min(center_y - height, img.shape[0]))))
    y_max = int(round(max(0, min(center_y + height, img.shape[0]))))
    x_min = int(round(max(0, min(center_x - width, img.shape[1]))))
    x_max = int(round(max(0, min(center_x + width, img.shape[1]))))
    # Cut the image
