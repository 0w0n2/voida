...
import { X, Camera, Grid, Home, Plus, UserRound } from 'lucide-react';
...
const CreateRoomModal = ({ onClose }: CreateRoomModalProps) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      await createRoom(title, category, thumbnail);
      onClose();
    } finally {
      setIsLoading(false);
    }
    ...
    <div css={overlay}>
      <div css={modal}>
        ...
        >
          {thumbnailImageUrl ? (
            <>
              <img src={thumbnailImageUrl} css={thumbnailImage} alt="썸네일" />
              <div
                css={[thumbnailOverlay, isDragging && thumbnailOverlayActive]}
              >
                {isDragging ? (
...
const spinner = css`
  ...
  border-top: 8 px solid var(--color-primary);
  border-radius: 50%;
