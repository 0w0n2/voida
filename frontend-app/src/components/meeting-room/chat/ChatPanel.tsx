...
export default function ChatPanel({ meetingRoomId }: ChatPanelProps) {
  const { chatMessages, setChatMessages, addChatMessage, clearChatMessages } =
    useMeetingRoomStore();
  ...
  useEffect(() => {
    const handleOutsideClick = (e: MouseEvent) => {
      if (!emojiAreaRef.current?.contains(e.target as Node))
  ...
  useEffect(() => {
    (async () => {
      ...
      connectStomp(meetingRoomId, (msg) => {
        ...
        addChatMessage(meetingRoomId, {
          ...msg,
          isMine: msg.senderUuid === myUuid,
        });
      });
    })();
    return () => {
    ...
    (async () => {
      try {
        ...
        const msgs = res.content
          .map((m) => ({ ...m, isMine: m.senderUuid === myUuid }))
          .reverse();
  ...
  const fetchOldMessages = useCallback(async () => {
    ...
    try {
      ...
      if (!res.content.length) {
        ...
        useAlertStore
          .getState()
          .showAlert('모든 채팅 기록을 불러왔습니다.', 'top');
        return;
      }
      const myUuid = useAuthStore.getState().user?.memberUuid;
      const msgs = res.content
