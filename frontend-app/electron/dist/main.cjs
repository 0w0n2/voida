...
var createOverlay = require('./overlayWindow.cjs').createOverlay;
var mainWin;
var overlayWin = null;
// 메인 창 만드는 함수
...
app.whenReady().then(function () {
    ...
    ipcMain.on('join-live', function (_, roomId) {
        if (overlayWin)
            overlayWin.close(); // 기존 창 있으면 제거
        overlayWin = createOverlay(roomId, true); // 새 오버레이 생성
        mainWin.hide(); // 필요 시 메인 창 숨기는거(오버레이만 남기고 싶을 때)
    });
    // 그냥 닫기
    ipcMain.on('leave-live', function () {
        overlayWin === null || overlayWin === void 0 ? void 0 : overlayWin.close();
        overlayWin = null;
