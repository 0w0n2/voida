...
import_electron2.app.whenReady().then(() => {
  ...
  import_electron2.ipcMain.on("open-overlay", (_e, init) => {
    ...
    win?.hide();
    const overlayWin2 = createOverlayWindow(isDev, overlayPosition, overlayTransparency);
    ...
    overlayWin2.webContents.once("did-finish-load", () => {
      overlayWin2.webContents.send("overlay:init", { roomId, overlayPosition, overlayTransparency });
    });
    overlayWin2.show();
    overlayWin2.focus();
  });
